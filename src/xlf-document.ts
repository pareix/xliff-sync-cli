...
export class XlfDocument {
  public get valid(): boolean {
    return !!this.root && (this.version === '1.2' || this.version === '2.0') && !!this.sourceLanguage;
  }
  ...
  public get translationUnitNodes(): XmlNode[] {
    ...
    switch (this.version) {
      ...
      case '2.0': {
        ...
        if (fileNode) {
          return <XmlNode[]>fileNode.children.filter((node) => typeof node !== 'string' && node.name === 'unit');
        }
        return [];
      }
      default:
  ...
  private getGroupTranslationUnitNodes(rootNode: XmlNode): XmlNode[] {
    let transUnits: XmlNode[] = [];
    const groupNodes: XmlNode[] = <XmlNode[]>rootNode.children.filter(
  ...
  private getTranslationUnitsFromRoot(rootNode: XmlNode): XmlNode[] {
    return <XmlNode[]>rootNode.children.filter((node) => typeof node !== 'string' && node.name === 'trans-unit');
  }
  ...
  private constructor() {
    this.developerNoteDesignation = DEFAULT_XLIFF_SYNC_SETTINGS.developerNoteDesignation;
    this.xliffGeneratorNoteDesignation = DEFAULT_XLIFF_SYNC_SETTINGS.xliffGeneratorNoteDesignation;
    this.preserveTargetAttributes = DEFAULT_XLIFF_SYNC_SETTINGS.preserveTargetAttributes;
    this.preserveTargetOrder = DEFAULT_XLIFF_SYNC_SETTINGS.preserveTargetAttributesOrder;
    this.preserveTargetChildNodes = DEFAULT_XLIFF_SYNC_SETTINGS.preserveTargetChildNodes;
    this.parseFromDeveloperNoteSeparator = DEFAULT_XLIFF_SYNC_SETTINGS.parseFromDeveloperNoteSeparator;
    this.parseFromDeveloperNoteTrimCharacters = DEFAULT_XLIFF_SYNC_SETTINGS.parseFromDeveloperNoteTrimCharacters;

    this.missingTranslation = DEFAULT_XLIFF_SYNC_SETTINGS.missingTranslation;
    if (this.missingTranslation === '%EMPTY%') {
      this.missingTranslation = '';
    }

    this.needsWorkTranslationSubstate = DEFAULT_XLIFF_SYNC_SETTINGS.needsWorkTranslationSubstate;
    this.addNeedsWorkTranslationNote = DEFAULT_XLIFF_SYNC_SETTINGS.addNeedsWorkTranslationNote;
    this.useSelfClosingTags = DEFAULT_XLIFF_SYNC_SETTINGS.useSelfClosingTags;
  }

  public static async loadFromPath(sourcePath: string): Promise<XlfDocument> {
    try {
      const source = await fs.readFile(sourcePath, 'utf8');
      return await this.load(source);
    } catch (ex) {
      const message = ex instanceof Error ? ex.message : String(ex);
      throw new Error(`${message}; File: ${sourcePath}`);
    }
  }
  ...
  public static async load(source: string): Promise<XlfDocument> {
    try {
      ...
      const message = ex instanceof Error ? ex.message : String(ex);
      throw new Error(`Failed to load XLIFF document, error: ${message}`);
    }
  }
  ...
  public findTranslationUnitByXliffGeneratorNoteAndSource(xliffGenNote: string, source: string): XmlNode | undefined {
  ...
  public findTranslationUnitByXliffGeneratorAndDeveloperNote(xliffGenNote: string, devNote: string): XmlNode | undefined {
    ...
    return this.translationUnitNodes.find(
      (node) => this.getUnitXliffGeneratorNote(node) === xliffGenNote && this.getUnitDeveloperNote(node) === devNote,
    );
  }

  public findTranslationUnitBySourceAndDeveloperNote(source: string, devNote: string | undefined): XmlNode | undefined {
    ...
    return this.translationUnitNodes.find(
      (node) =>
        this.getUnitSource(node) === source &&
        this.getUnitDeveloperNote(node) === devNote &&
        this.getUnitTranslationText(node) !== undefined,
    );
  }
  ...
  public findFirstTranslationUnitBySource(source: string): XmlNode | undefined {
    ...
    return this.translationUnitNodes.find(
      (node) => this.getUnitSource(node) === source && this.getUnitTranslationText(node) !== undefined,
    );
  }
  ...
  public getUnitXliffGeneratorNote(unitNode: XmlNode): string | undefined {
    ...
    if (xliffGenNode && xliffGenNode.children && xliffGenNode.children.length && typeof xliffGenNode.children[0] === 'string') {
  ...
  public getUnitDeveloperNote(unitNode: XmlNode): string | undefined {
    ...
    switch (this.version) {
      case '1.2':
        devNode = <XmlNode | undefined>unitNode.children.find(
          (node) =>
            typeof node !== 'string' && node.name === 'note' && node.attributes.from === this.developerNoteDesignation,
        );
        break;
      case '2.0': {
        const notesNode = this.getNode('notes', unitNode);
        if (notesNode) {
          devNode = <XmlNode | undefined>notesNode.children.find(
            (node) =>
              typeof node !== 'string' && node.name === 'note' && node.attributes.category === this.developerNoteDesignation,
          );
        }
        break;
      }
    ...
    if (devNode && devNode.children && devNode.children.length && typeof devNode.children[0] === 'string') {
  ...
  public getUnitTranslationFromDeveloperNote(unitNode: XmlNode): string | undefined {
    ...
    for (const translationEntry of translationEntries) {
      ...
      const language = translationEntry.substr(0, sepIdx);
  ...
  public mergeUnit(
    sourceUnit: XmlNode,
    targetUnit: XmlNode | undefined,
    translChildNodes: (XmlNode | string)[] | undefined,
  ): void {
    ...
    if (targetUnit) {
      ...
      if (this.preserveTargetChildNodes) {
        const lastNodeIdxReversed: number = sourceUnit.children
          .slice()
          .reverse()
          .findIndex((child) => typeof child !== 'string');
        let lastNodeIdx: number =
          lastNodeIdxReversed >= 0 ? sourceUnit.children.length - 1 - lastNodeIdxReversed : lastNodeIdxReversed;
          ...
          if (targetUnitChildNode.name) {
            ...
            switch (this.version) {
              case '1.2':
                ...
                if (!appendChildNode) {
                  appendChildNode =
                    ['source', 'seg-source', 'target', 'context-group', 'count-group', 'prop-group', 'note'].indexOf(
                      targetUnitChildNode.name,
                    ) < 0;
                }
                break;
            }
  ...
  public createTargetNode(
    parentUnit: XmlNode,
    attributes: { [key: string]: string },
    translChildNodes: (XmlNode | string)[],
  ): XmlNode {
    return {
      name: 'target',
      local: 'target',
  ...
  public appendTargetNode(unit: XmlNode, targetNode: XmlNode): void {
    ...
    switch (this.version) {
      case '1.2':
        sourceIdx = unit.children.findIndex((child) => typeof child !== 'string' && child.name === 'source');
        targetIdx = unit.children.findIndex((child) => typeof child !== 'string' && child.name === 'target');
      ...
      case '2.0': {
        ...
        if (segmentNode) {
          ...
          sourceIdx = segmentNode.children.findIndex((node) => typeof node !== 'string' && node.name === 'source');
          targetIdx = segmentNode.children.findIndex((node) => typeof node !== 'string' && node.name === 'target');

          if (targetIdx >= 0) {
            segmentNode.children[targetIdx] = targetNode;
          } else if (sourceIdx) {
            segmentNode.children.splice(sourceIdx + 1, 0, segmentNode.children[sourceIdx - 1], targetNode);
          } else {
  ...
  public getTargetAttribute(unit: XmlNode, attribute: string): string | undefined {
    ...
    if (targetNode && targetNode.attributes) {
      const attributeValue = targetNode.attributes[attribute];
  ...
  public getState(unit: XmlNode): translationState | undefined {
    const stateNode: XmlNode | undefined = this.tryGetStateNode(unit);
  ...
  public setState(unit: XmlNode, newState: translationState) {
    const stateNode = this.tryGetStateNode(unit);
    if (!stateNode && this.version === '1.2') {
      const attributes: { [key: string]: string } = {};
      this.updateStateAttributes(attributes, newState);
      const targetNode: XmlNode = this.createTargetNode(unit, attributes, []);
      this.appendTargetNode(unit, targetNode);
    } else if (stateNode) {
  ...
  public setXliffSyncNote(unit: XmlNode, noteText: string) {
    ...
    const noteAttributes: { [key: string]: string } = {};
    ...
    const targetIdx = unit.children.findIndex((child) => typeof child !== 'string' && child && child.name === 'target');
    if (noteIdx >= 0) {
  ...
  public tryDeleteXliffSyncNote(unit: XmlNode): boolean {
    const notesParent: XmlNode | undefined = this.getNotesParent(unit);
    if (notesParent) {
      const noteIdx: number = this.findXliffSyncNoteIndex(notesParent);
      if (noteIdx >= 0) {
        let deleteCount = 1;

        const newLineChild = notesParent.children[noteIdx + 1];
  ...
  private findXliffSyncNoteIndex(notesParent: XmlNode | undefined): number {
    const categoryAttributeValue = 'XLIFF Sync';
  ...
  private findNoteIndex(notesParent: XmlNode | undefined, categoryAttributeValue: string): number {
    ...
    return notesParent.children.findIndex(
      (child) =>
        ...
        child.name === 'note' &&
        child.attributes &&
        child.attributes[categoryAttributeName] === categoryAttributeValue,
    );
  }
